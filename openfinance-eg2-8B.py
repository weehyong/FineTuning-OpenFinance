...
ESTIMATED TIME
──────────────
  Qwen3-8B on DGX Spark:    ~60-90 minutes
  Qwen3-8B on A100:          ~45-60 minutes
  Qwen3-8B on Colab A100:   ~60-90 minutes
...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                                                                          ║
# ║   CHAPTER 1: WHY THIS APPROACH WORKS                                     ║
# ║                                                                          ║
# ╚════════════════════════════════════════════════════════════════════════════╝
#
...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                                                                          ║
# ║   CHAPTER 2: LOADING THE MODEL WITH UNSLOTH                              ║
# ║                                                                          ║
# ╚════════════════════════════════════════════════════════════════════════════╝
#
...
# Higher rank = more capacity = more memory. For our 8B model, rank 32
# is the right choice. Dragon LLM used rank 32–64 for their 8B models.
...
# Auto-detect available GPUs and repair CUDA_VISIBLE_DEVICES if needed.
# An empty string actively hides all GPUs; query nvidia-smi for the real
# list and set it, falling back to unsetting the variable entirely.
import subprocess as _sp

def _auto_fix_cuda():
    if os.environ.get("CUDA_VISIBLE_DEVICES", None) != "":
        return  # already set to something valid (or unset) — leave it alone
    try:
        out = _sp.run(
            ["nvidia-smi", "--query-gpu=index", "--format=csv,noheader"],
            capture_output=True, text=True, timeout=5,
        )
        indices = [l.strip() for l in out.stdout.strip().splitlines() if l.strip()]
        if indices:
            os.environ["CUDA_VISIBLE_DEVICES"] = ",".join(indices)
            return
    except Exception:
        pass
    del os.environ["CUDA_VISIBLE_DEVICES"]  # fallback: unset and let CUDA decide
...
class Config:
    # -- Model Selection --
    # We use Qwen3-8B, the instruction-tuned 8B model in the Qwen 3
    # family — matching the model Dragon LLM used. Starting from an instruct
    # model means the model already knows how to follow instructions and reason;
    # we only need to teach it finance.
    ...
    #
    # We bump to r=32 now that we are using the 8B model. Here is why it
    # matters: each LoRA adapter is a pair of low-rank matrices (A and B)
    # injected into a weight matrix W. The rank r controls how many
    # independent "directions" the adapter can express. A 0.6B model has
    # relatively narrow weight matrices, so rank 16 gives plenty of
    # expressive capacity. An 8B model has much wider weight matrices
    # (e.g. 4096-dim attention projections vs. 1024-dim), meaning there
    # are far more distinct financial concepts the adapter needs to capture.
    # Doubling the rank doubles the adapter's representational capacity,
    # at the cost of roughly doubling the number of trainable parameters
    # (still only ~1-2% of the full model). For Dragon LLM's 8B models,
    # r=32 is the standard recommendation and matches their published setup.
    lora_r: int = 32
    # alpha: Scaling factor. Setting alpha = r keeps the adapter's effective
    # learning rate consistent regardless of rank — a standard convention.
    lora_alpha: int = 32
    ...
    per_device_train_batch_size: int = 2  # Reduced from 4 → 2 for the 8B model.
    gradient_accumulation_steps: int = 8  # Increased from 4 → 8 to keep the same
    # ^ Effective batch size = 2 × 8 = 16 samples per weight update (unchanged).
    # The 8B model's hidden size (4096) produces much larger intermediate
    # activations than the 0.6B (1024). With batch=4 and packing enabled,
    # cuBLAS was receiving matrices with shapes it couldn't execute in BF16
    # (CUBLAS_STATUS_EXECUTION_FAILED). Halving the batch size reduces the
    # leading tensor dimension and resolves the issue while keeping training
    # throughput equivalent through doubled gradient accumulation.
    ...
    # Dragon LLM used tens of thousands of samples. We use 20,000 to get
    # close to their scale while staying within a ~60-75 minute run on the
    # DGX Spark. The ratios below match their published data mix exactly.
...
# Spark, this takes about 2-3 minutes for 8B. The model weights are frozen
...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                                                                          ║
# ║   CHAPTER 3: BUILDING THE TRAINING DATASET                               ║
# ║                                                                          ║
# ╚════════════════════════════════════════════════════════════════════════════╝
#
...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                                                                          ║
# ║   CHAPTER 4: FORMATTING DATA FOR TRAINING                                ║
# ║                                                                          ║
# ╚════════════════════════════════════════════════════════════════════════════╝
#
...
training_args = SFTConfig(
    ...
    packing=True,                    # Concatenates short examples to fill
    # each sequence up to max_seq_length, eliminating padding waste.
    # Our dataset has many short examples (sentiment labels, terminology,
    # math problems), so packing can give 30-50% faster training.
    # Previously disabled due to CUBLAS_STATUS_EXECUTION_FAILED — that was
    # caused by a torch/CUDA capability mismatch (torch 2.10 capped at 12.0,
    # GB10 is 12.1). Resolved by upgrading to torch nightly (2.12+).
...
print(f"  {'─' * 45}")
print(f"\n  Starting training... (this will take 60-90 minutes on DGX Spark)\n")
...
# could indicate overfitting. With 20,000 samples, expect ~0.8-1.2.


# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                                                                          ║
# ║   CHAPTER 6: TESTING THE FINE-TUNED MODEL                                ║
# ║                                                                          ║
# ╚════════════════════════════════════════════════════════════════════════════╝
#
...
#
# Note: with 20,000 training samples and an 8B model, do not expect
...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                                                                          ║
# ║   CHAPTER 7: SAVING AND EXPORTING THE MODEL                              ║
# ║                                                                          ║
# ╚════════════════════════════════════════════════════════════════════════════╝
#
...
print(f"""
  ...
  ────────────────
  1. SCALE THE DATA
     Increase total_samples from 20,000 to 50,000+. This single
     change will have the biggest impact on quality.

  2. ADD MORE EPOCHS
     Increase num_train_epochs from 1 to 2-3 for better convergence.
     With 20,000 samples, a second epoch is low overfitting risk.
